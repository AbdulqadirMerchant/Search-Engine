...
def search(event = None):
    ...
    try:
        ...
        for index, image_name in enumerate(pages.images):
            # -4: will retrieve the extension of the image
            if image_name[-4:] in [".jpg", ".png", ".gif"]:
                
                #It is retrieving the image from the value generated by pages.images
                #The second argument will be the name of the file
                
                urllib.request.urlretrieve(image_name, query+image_name[-4:])
                image = PIL.Image.open(query + image_name[-4:])
                image = image.resize((150, 150), PIL.Image.ANTIALIAS)
                image = ImageTk.PhotoImage(image)
                previous_query[query] = image_name[-4:]
                search_image.config(image=image)
                #Just in case the image doesn't show up
